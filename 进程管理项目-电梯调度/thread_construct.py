    ...
    def arrange2(self):
        for idxfloor, i in enumerate(reversed(self.ele_group_task[1])):
            if i == 1:
                ...
                min_static_index = -1
                tag = 4
                idxfloor = 19 - idxfloor
                if self.button_test[1][idxfloor] == 1:
                ...
                for idx, e0 in enumerate(self.ele):
                    ...
                    if e0.status == 0:
                        ...
                        if e0.floor > idxfloor and (e0.floor - idxfloor) < min_working:
                            min_working = idxfloor - e0.floor
                            min_working_index = idx
                    elif e0.status == 1 and e0.floor < idxfloor + 5:
                        if e0.target_status != 1: # and max(self.ele_task[idx]) > idxfloor + 1:
                            if max(self.ele_task[idx]) <= idxfloor + 1:
                                self.ele_group_task[1][max(self.ele_task[idx]) - 1] = 1
                                self.button_test[1][max(self.ele_task[idx]) - 1] = 0
                                self.ele_task[idx].pop(-1)
                                self.ele_task[idx].append(idxfloor + 1)
                            tag = -1
                            break
                        else:
                            tag -= 1
